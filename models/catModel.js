...
const updateCat = async (id, name, weight, owner, birthdate, next) => {
  console.log(id, name, weight, owner, birthdate);
  try {
    const [rows] = await promisePool.execute('UPDATE wop_cat SET name = ?, weight = ?, owner = ?, birthdate = ? WHERE cat_id = ?', [name, weight, owner, birthdate, id]);
    return rows;
  } catch (e) {
